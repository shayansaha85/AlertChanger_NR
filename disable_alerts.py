...
def DISABLE_ALERTS():
    def alert_disable_email_received(foldername):
        ...
        status, messages = mail.search(None, f'(UNSEEN FROM "{SENDER_EMAIL}" SUBJECT "{SUBJECT_KEYWORD_DISABLE}")')
    ...
    if alert_disable_email_received(FOLDERNAME):
        print()
        print("EMAIL FOR DISABLING ALERTS IS RECEIVED")
        for condition in conditionIDs:
            disableAlertsBaseline(condition)
            disableAlertsStatic(condition)
            print(f">> Alert Disabled For {condition}")
        print()
        print("------ WORK DONE ------")
        print()
    else:
        print("EMAIL NOT RECEIVED FOR DISABLING THE ALERTS")

scheduler = BlockingScheduler()
scheduler.add_job(DISABLE_ALERTS, 'interval', seconds = 5)
scheduler.start()
